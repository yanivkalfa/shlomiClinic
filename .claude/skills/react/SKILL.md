...
- **[2026-08-13] Hebrew in generated PDFs** — jsPDF's built-in fonts have no Hebrew glyphs and no RTL shaping, and embedding a Unicode font is heavy. Paint the page to a `<canvas>` (set `ctx.direction = 'rtl'`, `ctx.textAlign = 'right'`, system fonts render Hebrew correctly) and `addImage` that canvas into the PDF. Keeps exports fully translated. See `POC/src/pdf.js`. (Context: signed-documents download.)
- **[2026-08-13] Dirty-form navigation blocking without a router** — keep a registry of `{formId: resetFn}` in the store; `navigate()` checks it and stashes a `pendingNav` instead of moving, a popup offers leave/stay, and "leave" calls every reset then replays the nav. Mirror the registry into a `useRef` — `navigate` is memoised and would otherwise read a stale snapshot. (Context: react-hook-form `useGuardedForm` + `LeaveGuardPopup`.)
- **[2026-08-13] Cancel in-flight queries on navigation** — call `queryClient.cancelQueries()` inside the navigate action and set `retry: false` globally, otherwise an aborted request retries and resolves into the page the user already left. (Context: react-query weather + calendar feed.)
